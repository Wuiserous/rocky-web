export const GOOGLE_CALLBACK_URL = "https://getrockyapp.com/oauth/google/callback";
...
export function isSupportedGoogleProvider(provider) {
  return Object.hasOwn(GOOGLE_PROVIDER_SCOPES, provider);
}

export function getOAuthStateKey(state) {
  return `rocky_oauth_state:${state}`;
}
...
export function isValidDesktopRedirectUri(value) {
  try {
    ...
    return (
      ...
      Boolean(url.port) &&
      url.pathname === "/rocky/oauth/callback"
    );
  } catch {
    return false;
  }
...
export function envReady(...names) {
